...
#by removing the # in the beginning of the code
...
# **************************************************************
# Part 1: Specific Quentions/Tasks
...
get_data <- function(idpractice) {
  q1a <- paste0(" select practiceid, bnfname as PRESCRIBED_MED, actcost as MED_PRICE, quantity as TOTAL_COST
                  from gp_data_up_to_2015 as gpdata
                  where gpdata.practiceid = '", idpractice, "'
                  group by 1,2,3,4
                  order by sum(actcost) desc
                  limit 5
                  ")
...
#TIDYVERSE
#I gave this table from the database to tidy it up
tidied_data <- tbl(con, "gp_data_up_to_2015")

#here I tried to tidy the data in practice ID column
#it should clean the na or missing values data from the practice ID column
tidied_data %>%
  filter(practiceid == "^W\\d{5}$")
#filter is the function used in tiday verse to filter the data accordingly
#so here it takes the vales which only starts with W and followed by 5 digits
#values like wyz,w89238 will be invalid
...
topfivedrugs <- get_data(idpractice)
print(topfivedrugs)
View(topfivedrugs)
...
#width is how much the width of teh bar shoudl be
#bar plot
ggplot(medspend_illness, aes(x=illness_id, y=spend_on_med))+
...
#This concludes the open-ended analysis.
